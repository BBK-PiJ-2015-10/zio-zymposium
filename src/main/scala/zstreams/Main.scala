...
import zio.stream._
import zio.{Chunk, ChunkBuilder, Console, IO, Random, Scope, UIO, ZIO, ZIOAppArgs, ZIOAppDefault, ZNothing}
...
object Main extends ZIOAppDefault {
  ...
  val runViaPipes3Other =
    for {
      shared <- randomNums.broadcast(2, 16)
      s1 = shared(0) >>> doubler >>> filter >>> collectTen
      s2 = shared(1) >>> doubler >>> filter >>> collectFive
  ...
  //one more https://www.youtube.com/watch?v=3EO0yVf63xI
